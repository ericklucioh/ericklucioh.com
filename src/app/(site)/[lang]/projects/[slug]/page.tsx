...
export default async function ProjectCasePage({
	...
	return (
		<main className="mx-auto max-w-4xl p-6 md:p-10">
			...
			<div className="mb-8 grid gap-3 sm:grid-cols-3">
				{item.impact.length ? (
					item.impact.map((metric) => (
						<div key={metric.label} className="rounded-xl border border-[var(--text-third)] p-4">
			...
			<figure className="mb-8 overflow-hidden rounded-2xl border border-[var(--text-third)] p-3">
				<Image
					src={item.image}
			...
			<section className="space-y-4">
				{localizedSections.length ? (
					localizedSections.map((section) => (
						<article key={section.title} className="rounded-2xl border border-[var(--text-third)] p-5">
							<h2 className="mb-2 text-xl font-semibold">{section.title}</h2>
							<p className="text-[var(--text-secondary)]">{section.body}</p>
						</article>
					))
				) : (
					<article className="rounded-2xl border border-dashed border-[var(--text-third)] p-5">
